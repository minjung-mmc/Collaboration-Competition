...
Run `Tennis.ipynb`, you can either train the agent by yourself by setting `state` = 'Train' or test the saved agent model by setting `state` = 'Test' where you can see agent action slowly by setting `mode` = 'slow' or quickly by setting `mode` = 'fast'.
