...
class MultiAgents():
    ...
    def learn(self, experiences, agent, gamma):
        ...
        actions_pred = [agent_action_pred if j==agent.index.cpu().numpy()[0] else actions.index_select(1, torch.tensor([j]).to(device)).squeeze(1) for j, agent_j in enumerate(self.ma)]
